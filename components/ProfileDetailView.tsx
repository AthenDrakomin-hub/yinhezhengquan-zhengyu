              ...
              {group.items.map((item, i) => (
                <div key={i} className="flex items-center justify-between p-4 border-b border-[var(--color-border)] last:border-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {item.editable && <ICONS.ArrowRight size={10} className="text-[var(--color-text-muted)]" />}
                    {item.copyable && <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--color-text-muted)]"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>}
                  </div>
                </div>
              ))}
            </div>
