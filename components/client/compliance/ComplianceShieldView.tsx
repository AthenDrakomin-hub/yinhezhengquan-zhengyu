...
import { ICONS, MOCK_TICKETS } from '../../../lib/constants';
          ...
          <div className="relative z-10 space-y-6">
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] font-mono font-medium text-[var(--color-text-primary)]">2026.03.26 14:30</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-[var(--color-bg)] rounded-lg border border-[var(--color-border)]">
                ...
                <p className="text-xs font-medium text-[var(--color-primary)]">C3-稳健型</p>
              </div>
              <div className="p-3 bg-[var(--color-bg)] rounded-lg border border-[var(--color-border)]">
                <p className="text-[8px] text-[var(--color-text-muted)] font-bold uppercase mb-1">实名认证</p>
                <p className="text-xs font-medium text-[var(--color-primary)]">已认证 (*振华)</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="galaxy-card divide-y divide-[var(--color-border)] overflow-hidden">
            {[
              ...
              <div key={i} className="flex justify-between items-center p-4">
                ...
                <span className="text-[9px] font-medium text-[var(--color-primary)] px-2 py-0.5 rounded bg-[var(--color-primary)]/5 border border-[var(--color-primary)]/20 uppercase">
                  {item.status}
                </span>
              </div>
            ))}
             ...
             {MOCK_TICKETS.map(ticket => (
               <div key={ticket.id} className="galaxy-card p-4 flex justify-between items-center group cursor-pointer hover:border-[var(--color-primary)]/30 transition-all">
                  ...
                  <span className={`text-[8px] font-medium px-1.5 py-0.5 rounded border ${
                    ticket.status === 'CLOSED' ? 'bg-[var(--color-surface)] text-[var(--color-text-muted)] border-[var(--color-border)]' : 'bg-[var(--color-primary)]/10 text-[var(--color-primary)] border-[var(--color-primary)]/20'
                  }`}>
