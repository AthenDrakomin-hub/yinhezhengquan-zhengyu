      ...
      <div className="hidden md:block bg-slate-50 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 md:px-12 h-10 flex justify-between items-center text-[11px] font-medium text-slate-500">
          <div className="flex gap-6">
            ...
            <span onClick={() => navigate('/training-camp')} className="hover:text-[#E30613] cursor-pointer transition-colors">加入我们</span>
            <span onClick={() => navigate('/admin/login')} className="hover:text-[#E30613] cursor-pointer transition-colors">内部运营</span>
          </div>
          <div className="flex gap-6">
            <a href="https://www.chinastock.com.cn/newsite/online/branchNetwork.html" target="_blank" rel="noopener noreferrer" className="hover:text-[#E30613] cursor-pointer transition-colors">站点地图</a>
            <span onClick={() => navigate('/support')} className="hover:text-[#E30613] cursor-pointer transition-colors">联系我们</span>
            <span onClick={() => setShowAccessibility(true)} className="hover:text-[#E30613] cursor-pointer transition-colors">无障碍辅助</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-0 border border-slate-100 rounded-xl overflow-hidden shadow-sm">
            <div className="lg:w-1/2 aspect-video bg-slate-100 relative group cursor-pointer">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <button 
                  className={`pointer-events-auto w-20 h-20 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center transition-all hover:bg-white/30 ${isVideoPlaying ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
                  onClick={(e) => {
                    e.stopPropagation();
                    if (videoRef.current) {
                      videoRef.current.play();
                ...
                <button 
                  className={`pointer-events-auto w-20 h-20 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center transition-all hover:bg-white/30 ${isVideoPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                  onClick={(e) => {
                    e.stopPropagation();
                    if (videoRef.current) {
                      videoRef.current.pause();
