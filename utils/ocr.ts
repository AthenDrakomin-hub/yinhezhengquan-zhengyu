...
class IdCardOcr {
  ...
  async initialize(options: OcrOptions = {}): Promise<void> {
    ...
    try {
      // 创建 Worker - 使用 Supabase Storage 加载
      const language = options.language || 'chi_sim+eng';
      const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
      
      this.worker = await createWorker(language, 1, {
        // 从 Supabase Storage 加载核心库
        corePath: `${supabaseUrl}/storage/v1/object/public/ai-models/tesseract-core.wasm.js`,
        // 从 Supabase Storage 加载语言训练数据
        langPath: `${supabaseUrl}/storage/v1/object/public/ai-models`,
      ...
      console.log('OCR Worker 初始化成功（使用 Supabase Storage）');
    } catch (error) {
