...
async function fixAndCreateAccounts() {
  console.log('🔧 修复 profiles 表约束...\n');

  // 1. 修复触发器函数
  console.log('📋 步骤 1: 修复 handle_new_user 函数...');
  await sql`
  ...
  // 2. 修复现有数据
  console.log('📋 步骤 2: 修复现有 risk_level 数据...');
  await sql`
    UPDATE profiles 
    SET risk_level = 'C3' 
    WHERE risk_level NOT IN ('C1', 'C2', 'C3', 'C4', 'C5')
  `;
  console.log('✅ 数据修复成功\n');

  // 3. 创建用户
  console.log('📋 步骤 3: 创建初始账号...\n');
