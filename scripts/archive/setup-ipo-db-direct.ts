...
const DATABASE_URL = 'postgresql://postgres:GDragon19888.%E3%80%82@db.rfnrosyfeivcbkimjlwo.supabase.co:5432/postgres?sslmode=require';
...
async function setupIPOSync() {
  try {
    ...
    console.log('\n3️⃣ 创建访问策略...');
    await client.query(`
      CREATE POLICY IF NOT EXISTS "Allow public read access" ON public.ipo_sync_log
      FOR SELECT USING (true)
    `);

    await client.query(`
      CREATE POLICY IF NOT EXISTS "Allow service role write" ON public.ipo_sync_log
      FOR ALL USING (
