...
const transformTopic = (db: DbEducationTopic): EducationTopic => ({
  ...
  image: db.image || '',
  duration: db.duration ? `${db.duration}分钟` : '',
  content: db.content,
  order: db.order ?? 0,
...
export async function updateEducationTopic(
  ...
  if (topic.category !== undefined) updateData.category = topic.category;
  if (topic.difficulty !== undefined) updateData.difficulty = topic.difficulty;
  if (topic.author !== undefined) updateData.author = topic.author;
  if (topic.image !== undefined) updateData.image = topic.image;
  if (topic.is_published !== undefined) updateData.is_published = topic.is_published;
  if (topic.order !== undefined) updateData.order = topic.order;
  
  // 处理duration转换：string -> number
  if (topic.duration !== undefined) {
    const match = topic.duration.match(/(\d+)/);
    updateData.duration = match ? parseInt(match[1], 10) : 0;
  }
